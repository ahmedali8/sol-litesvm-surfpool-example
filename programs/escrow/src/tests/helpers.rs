...
pub fn setup_escrow_test() -> EscrowTestEnvironment {
    ...
    let mut wallets = create_wallets(&mut litesvm, 2, 1_000_000_000).unwrap();
    ...
    let alice_token_account_a =
        create_associated_token_account(&mut litesvm, &alice.pubkey(), &token_mint_a, &mint_authority).unwrap();
    let alice_token_account_b =
        create_associated_token_account(&mut litesvm, &alice.pubkey(), &token_mint_b, &mint_authority).unwrap();
    let bob_token_account_a =
        create_associated_token_account(&mut litesvm, &bob.pubkey(), &token_mint_a, &mint_authority).unwrap();
    let bob_token_account_b =
        create_associated_token_account(&mut litesvm, &bob.pubkey(), &token_mint_b, &mint_authority).unwrap();
